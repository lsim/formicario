...
describe('Brain Array Synchronization', () => {
  it('should maintain sync between square.numAnts and brain array length', () => {
    ...
    let brainCheckResults: { numAnts: number; numBrains: number }[] = [];
    ...
    brainCheckResults.forEach((result, index) => {
      expect(result.numAnts, 
        `Check ${index}: square.numAnts (${result.numAnts}) should equal brain array length (${result.numBrains})`).toBe(result.numBrains);
    });
  });
  ...
  it('should handle base building brain consistency', () => {
    ...
    const baseBuildingAnt: AntFunction = ((squareData?: SquareData[], antInfo?: AntInfo) => {
      ...
      console.log(`Brain consistency check: numAnts=${squareData[0].numAnts}, numBrains=${antInfo.brains.length}`);
      expect(squareData[0].numAnts, 
        `Brain mismatch: numAnts=${squareData[0].numAnts}, numBrains=${antInfo.brains.length}`).toBe(antInfo.brains.length);
