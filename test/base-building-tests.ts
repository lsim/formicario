...
describe('Base Building Brain Management', () => {
  it('should maintain consistent brain arrays during base building', () => {
    ...
    const baseBuildingAnt: AntFunction = ((squareData?: SquareData[], antInfo?: AntInfo) => {
      ...
      console.log(`Ant function called! Square: ants=${squareData[0].numAnts}, food=${squareData[0].numFood}, base=${squareData[0].base}`);

      // Check for undefined brains that would cause errors
      if (!antInfo.brains[0]) {
        errorCount++;
        console.error(`ERROR: Undefined brain[0] detected! Square has ${squareData[0].numAnts} ants, base: ${squareData[0].base}`);
        return 0;
      }
      ...
      if (!baseBuildingDetected) {
        ...
        console.log(`FORCING base building with ${squareData[0].numAnts} ants and ${squareData[0].numFood} food`);
        return 16; // Build base - this should trigger the brain consistency issue
      } else {
        // Otherwise just stay put
        console.log(`Not building base: base=${squareData[0].base}, ants=${squareData[0].numAnts}, food=${squareData[0].numFood}`);
        return 0;
      }
    }) as AntFunction;
  ...
  it('should handle ant creation at new bases correctly', () => {
    ...
    const newBaseTestAnt: AntFunction = ((squareData?: SquareData[], antInfo?: AntInfo) => {
      ...
      if (!antInfo.brains[0]) {
        brainErrorCount++;
        console.error(`Brain error on square with ${squareData[0].numAnts} ants, base: ${squareData[0].base}`);
        return 0;
      }
